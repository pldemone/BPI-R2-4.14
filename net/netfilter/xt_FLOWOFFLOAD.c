...
xt_flowoffload_check_hook(struct flow_offload *flow, void *data)
{
	...
	hlist_for_each_entry(hook, &table->hooks, list) {
		...
		if (hook->ops.dev->ifindex != tuple->iifidx &&
		    hook->ops.dev->ifindex != tuple->offload_iifidx &&
				...
				struct net_device **hw_dev)
{
	...
	for (i = 0; i <= stack.num_paths; i++) {
		...
		if (!is_valid_ether_addr(route->tuple[dir].out.h_source)) {
	...
	*flow_dev = dev;
	*hw_dev = (struct net_device *)stack.path[stack.num_paths - 1].dev;
	route->tuple[!dir].in.ifindex = dev->ifindex;
	route->tuple[!dir].in.offload_ifindex = (*hw_dev)->ifindex;
}
...
xt_flowoffload_route(struct sk_buff *skb, const struct nf_conn *ct,
		     ...
		     struct net_device **dev, struct net_device **hw_dev)
{
	int ret;
	...
	xt_flowoffload_route_check_path(route, ct, dir, &dev[!dir], &hw_dev[!dir]);
	xt_flowoffload_route_check_path(route, ct, !dir, &dev[dir], &hw_dev[dir]);
...
flowoffload_tg(struct sk_buff *skb, const struct xt_action_param *par)
{
	...
	if (xt_flowoffload_route(skb, ct, par, &route, dir, devs, hw_devs) < 0)
		goto err_flow_route;
	...
	if (table->ft.flags & NF_FLOWTABLE_HW_OFFLOAD) {
	    xt_flowoffload_check_device(table, hw_devs[0]);
	    xt_flowoffload_check_device(table, hw_devs[1]);
	}
